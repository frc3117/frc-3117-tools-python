...
def __float_event__(key):
    def e(event: ntcore.Event):
        key.value = event.data.value.getFloat()

    return e


class StreamDeckKeyFloat(StreamDeckKey):
    ...
    def bind_nt(self, nt_path: str):
        ...
        inst.addListener(self.__nt_entry, ntcore.EventFlags.kValueAll, __float_event__(self))
